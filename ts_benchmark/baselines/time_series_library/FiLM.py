...
class FiLM(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pred_len = config.seq_len if config.pred_len == 0 else config.pred_len

        self.seq_len_all = self.seq_len + self.label_len

        self.output_attention = config.output_attention
        self.layers = config.e_layers
        self.enc_in = config.enc_in
        self.e_layers = config.e_layers
        # b, s, f means b, f
        self.affine_weight = nn.Parameter(torch.ones(1, 1, config.enc_in))
        self.affine_bias = nn.Parameter(torch.zeros(1, 1, config.enc_in))
        ...
        if self.task_name == "classification":
            ...
            self.projection = nn.Linear(
                config.enc_in * config.seq_len, config.num_class
            )
