...
class Pyraformer(nn.Module):
    ...
    def __init__(self, config, window_size=[4, 4], inner_size=5):
        """
        ...
        if self.task_name == "short_term_forecast":
            window_size = [2, 2]
        self.encoder = Encoder(config, window_size, inner_size)
        ...
        if (
            ...
            self.projection = nn.Linear(
                (len(window_size) + 1) * self.d_model, self.pred_len * config.enc_in
            )
        elif self.task_name == "imputation" or self.task_name == "anomaly_detection":
            self.projection = nn.Linear(
                (len(window_size) + 1) * self.d_model, config.enc_in, bias=True
            )
        elif self.task_name == "classification":
            self.act = torch.nn.functional.gelu
            self.dropout = nn.Dropout(config.dropout)
            self.projection = nn.Linear(
                (len(window_size) + 1) * self.d_model * config.seq_len, config.num_class
            )
