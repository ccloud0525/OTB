    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            ...
            if (self.seq_len + self.pred_len) % period != 0:
                len = (((self.seq_len + self.pred_len) // period) + 1) * period
                padding = torch.zeros(
                    [x.shape[0], (len - (self.seq_len + self.pred_len)), x.shape[2]]
                ).to(x.device)
                out = torch.cat([x, padding], dim=1)
            else:
                len = self.seq_len + self.pred_len
                out = x
            # reshape
            out = (
                out.reshape(B, len // period, period, N)
                .permute(0, 3, 1, 2)
                .contiguous()
            )
