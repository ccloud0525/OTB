...
class LightTS(nn.Module):
    ...
    def __init__(self, config, chunk_size=24):
        """
        ...
        if (
            ...
            or config.task_name == "short_term_forecast"
        ):
            self.chunk_size = min(config.pred_len, config.seq_len, chunk_size)
        else:
            self.chunk_size = min(config.seq_len, chunk_size)
