...
class MICN(nn.Module):
    ...
    def __init__(self, config, conv_kernel=[12, 16]):
        """
        ...
        for ii in conv_kernel:
            if ii % 2 == 0:  # the kernel of decomposition operation must be odd
                ...
                isometric_kernel.append((config.seq_len + config.pred_len + ii) // ii)
            else:
                decomp_kernel.append(ii)
                isometric_kernel.append(
                    (config.seq_len + config.pred_len + ii - 1) // ii
                )
        ...
        if (
            ...
            # refer to DLinear
            self.regression = nn.Linear(config.seq_len, config.pred_len)
            self.regression.weight = nn.Parameter(
                (1 / config.pred_len) * torch.ones([config.pred_len, config.seq_len]),
                requires_grad=True,
            )
        ...
        if self.task_name == "classification":
            ...
            self.projection = nn.Linear(config.c_out * config.seq_len, config.num_class)
