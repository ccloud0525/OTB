...
def extract_info_and_merge(
    log_files_list: pd.DataFrame, results_of_evaluated_algorithm: pd.DataFrame,
...
def filter_data_and_calculate_result(
    ...
    selected_df["model_and_params"] = selected_df["model_name"] + ";" + selected_df["model_params"]
...
def report(report_config: dict) -> None:
    ...
    - report_config (dict): A dictionary containing the following keys and their respective values:
        ...
        - saved_report_file_name (str): The name for the saved report file.
    ...
    evaluated_algorithm_columns = pd.read_csv(
        os.path.join(ROOT_PATH, report_config["log_files_list"][0])
    ).columns.values
    for log_file in report_config["log_files_list"][1:]:
        ...
        if not (evaluated_algorithm_columns == log_file_columns).all():
            raise ValueError(
                f'Column names are inconsistent between {log_file} and {report_config["log_files_list"][0]}'
            )
    ...
    if isinstance(report_config["report_metrics"], str):
        report_config["report_metrics"] = [report_config["report_metrics"]]

    final_result = []
    many_null_metrics_nums = 0
    cols = []
    for metric_name in report_config["report_metrics"]:
        # 填补缺失值
        nan_count = results_of_evaluated_algorithm[metric_name].isna().sum()
        if nan_count > float(report_config["null_value_threshold"]) * len(
        ...
        (
            ...
            data_filename_list_of_evaluated_algorithm,
            adjusted_df,
        ) = extract_info_and_merge(
            report_config["log_files_list"], results_of_evaluated_algorithm
        )
    ...
    if many_null_metrics_nums == len(report_config["report_metrics"]):
        raise ValueError(
        ...
        result_df.to_csv(
            os.path.join(ROOT_PATH, "result", report_config["saved_report_file_name"]),
            index=False,
        )
